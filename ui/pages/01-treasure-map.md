...
- 用户位置标记：44×44 白色圆底 + 项圈色 3px 边框 + 用户宠物头像（🐶 占位）+ 项圈色脉冲环
...
### 开始寻宝按钮（今日任务未开始时显示）
- 地图中下方（Tab 栏上方 40px），水平居中，圆形，直径 80px，项圈色渐变背景（collar-lt → collar）
- 白色 🐾 图标 32px 居中，下方"开始寻宝" 13px 白色加粗，间距 8px
...
### 任务摘要卡片（开始寻宝按钮展开）
- 按钮原地变形为圆角矩形（屏幕 75% 宽，圆角 20px）
- 动画序列（~0.6s）：缩放反馈 → 光环扩散淡出 → 圆形变矩形 → 背景过渡为毛玻璃（白 85% + backdrop-blur 16px） → 内容淡入
- 标题行：🐾 "今日寻宝任务" 15px 加粗 + 右侧 "0/3" 项圈色
- 3 个任务摘要行（每行 ~36px）：圆形图标（12px 项圈色描边）+ 任务名 14px + 奖励 13px #999
- "出发"按钮：项圈色渐变，白字 15px 加粗，40px 高，圆角 20px，全宽
- 点击外区域关闭：反向收缩回按钮（0.4s）
- 低端机降级：跳过变形动画，按钮淡出 → 卡片淡入（0.3s）
...
- 入场：从底部滑入，衔接出发按钮收起动画（0.3s，delay 0.2s）
- 点击展开任务卡片

### 任务卡片（浮动 Bottom Sheet）
- 从底部滑出，约屏幕 1/3 高度
- 左右 24px 边距（约屏幕 85% 宽），水平居中，四边圆角 16px
- 底部距 Tab 栏 12px
- 白色背景，拖拽条（36×4px，#F0F0F0）
...
- **默认（未开始）：** 地图加载完成，显示附近宝箱，中央显示开始寻宝按钮（光环脉冲）
- **默认（进行中）：** 地图 + 宝箱 + 任务条显示进度
...
PawQuest 自定义品牌设计系统。全屏 Apple Maps 风格地图为主体，所有 UI 元素以毛玻璃浮层覆盖其上（低端机降级为半透明纯色），营造沉浸式探索感。视觉基调轻盈圆润——大圆角、柔和阴影、暖白表面。宝箱通过弹跳、发光、粒子等微动画传递"可交互"和"奖励"的反馈。项圈色作为个性化视觉线索贯穿开始寻宝按钮、任务条、活动按钮和任务宝箱。整体克制，不喧宾夺主于地图本身。iOS 与 Android 视觉完全统一，仅导航交互跟随平台惯例。
